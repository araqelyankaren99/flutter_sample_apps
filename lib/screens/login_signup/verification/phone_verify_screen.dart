...
import 'package:flutter_sample_apps/screens/login_signup/shared/verification/pin_code_fields.dart';
...
import 'package:flutter_sample_apps/screens/profile/shared/timer.dart';
...
class PhoneVerifyScreen extends StatefulWidget {
  const PhoneVerifyScreen({required this.loginType, required this.phoneNumber});
...
class _PhoneVerifyScreenState extends State<PhoneVerifyScreen> {
  ...
  Widget _renderPinField() {
    return GestureDetector(
        ...
        child: PinCodeFields(
            ...
            onCompleted: (pinCode) => _onPinCodeCompleted(pinCode),),);
  }
  ...
  Future<void> _otpDispose() async {
    if (_isAndroidPlatform() && _controller is OTPTextEditController) {
      await (_controller as OTPTextEditController).stopListen();
    }
  }
...
extension _PhoneVerifyScreenListener on _PhoneVerifyScreenState {
  void _listener(BuildContext context, state) {
    ...
    if (state is GetPhoneNumberLoadingState) {
      _onClearPinCode();
    ...
    if (state is ResendCodeFailedState) {
      ...
      },).cancelAction(context, 'Send Failed');
    }
    ...
    if (state is PinCodeVerifySuccessedState) {
      ...
      if (widget.loginType == LoginType.changeNumber) {
        ...
        if (profileinformationBloc != null) {
          profileinformationBloc.add(DriverPhoneNumberChangedEvent(
